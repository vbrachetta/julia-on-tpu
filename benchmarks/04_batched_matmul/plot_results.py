"""
plot_results.py - Plot batched matrix multiplication benchmark comparison
...
fig.suptitle("Batched Matrix Multiplication (n=1024): Julia/Reactant vs Python/JAX on TPU v4 — GFLOPS",
...
for ax, prec in zip(axes, precisions):
    ...
    ax.plot(j["batch"], j["gflops"], marker="o", label="Julia/Reactant",
...
fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=False)
fig.suptitle("Batched Matrix Multiplication (n=1024): Julia/Reactant vs Python/JAX on TPU v4 — Wall-Clock Time",
             fontsize=13, fontweight="bold")
...
for ax, prec in zip(axes, precisions):
    ...
    ax.plot(j["batch"], j["time_ms"], marker="o", label="Julia/Reactant",
            color="royalblue", linewidth=2)
